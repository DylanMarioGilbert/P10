...
const Select = ({
  ...
  const [value, setValue] = useState(null);  // Initialize with null
  const [collapsed, setCollapsed] = useState(true);
  const changeValue = (newValue) => {
    onChange(newValue);  // Pass newValue to onChange
    setValue(newValue);
    setCollapsed(true);  // Close the dropdown after selecting a value
  };
  return (
    <div className={`SelectContainer ${type}`} data-testid="select-testid">
      ...
      <div className="Select">
        <ul>
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"} onClick={() => setCollapsed(false)}>
